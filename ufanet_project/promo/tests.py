...
from django.test import Client, TestCase  # Импорт для обычных тестов
...
class OfferAPITestCase(APITestCase):
    ...
    def test_order_offers(self):
        ...
        discounts = [
            (
                int(item["discount"].strip("%"))
                if isinstance(item["discount"], str) and item["discount"].endswith("%")
                else int(item["discount"])
            )
